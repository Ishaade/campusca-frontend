...
function CreateQuiz() {
  ...
  const addQuestion = () => {
    const newQuestion = {
      id: Math.max(...formData.questions.map(q => q.id)) + 1,
      question: '',
      type: 'multiple-choice',
      options: ['', '', '', ''],
      correctAnswer: 0,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!formData.title.trim() || formData.title.trim().length < 3) {
        setError('Quiz title is required and must be at least 3 characters');
        return;
      }

      if (formData.questions.some(q => !q.question.trim() || q.question.trim().length < 3)) {
        setError('All questions must have text (minimum 3 characters)');
        return;
      }
      ...
      if (formData.scheduledStart && formData.scheduledEnd) {
        ...
        // Warn if timeLimit exceeds scheduled window
      ...
      await apiRequest(`/api/quizzes/rooms/${roomId}`, {
        method: 'POST',
        body: JSON.stringify(payload)
      });

      setSuccess('Quiz created successfully!');
      setTimeout(() => navigate(`/room-details/${roomId}`), 1500);

    } catch (err) {
      // If the API returned validation details, show them to the user
      if (err?.details && Array.isArray(err.details)) {
        // Build a friendly message from details (Zod errors)
        const msgs = err.details.map(d => d.message || (d.path ? `${d.path.join('.')} failed` : String(d))).slice(0,5);
        setError(`Failed to create quiz: ${msgs.join('; ')}`);
      } else {
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                <div className="form-group">
                  ...
                  <input
                    ...
                    max="10"
                  />
                  <p className="text-sm text-gray-500 mt-1">Number of times a student can attempt this quiz.</p>
                </div>
                <div className="form-group">
